...
inline tcp_itch_connection_ref_t  getConnection<tcp_itch_connection_ref_t>(::fastapp::App & app)
{
    ...
    tcp_itch_connection_ref_t  session(tsession);
...
inline udp_itch_connection_ref_t getConnection<udp_itch_connection_ref_t>(::fastapp::App &  app)
{
    ...
    udp_itch_connection_ref_t  session(usession);
...
void initLogging(auto & sessionRef, auto & callbacks)
{
    auto * processor = sessionRef.getProcessor();

    // print HB, etc.
    processor->registerAllOn
    (
         [] () {return true; }
    );
    ...
    sessionRef.getSession().setPrintFlag(PrintFlags::ClientHrtBeat | PrintFlags::ServerHrtBeat | PrintFlags::Trace | PrintFlags::Book);
...
int run_market(::fastapp::App & app)
{
    ...
    initLogging(marketRef, callbacks);
